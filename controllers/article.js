...
async function getArticleById(ctx) {
  const { labelId } = ctx.query;
  console.log(ctx);
  if (!labelId) {
    miss(ctx, "labelId不能为空");
    return;
  }
  const sql = `SELECT id, title, label_name, cover_img, b.create_time AS create_time, b.update_time AS update_time FROM label a inner join article b on a.label_id = b.label_id  WHERE b.label_id = ${labelId}`;
  const result = await query(sql);
  success(ctx, result);
}
